...
exports.fileNotFound = (req, res) => {
    // Error 404 Handler
    res.status(404).send({status: 404, error: "Error 404! File Not Found", home: homeurl});
};
...
exports.psqlErrorHandling = (err, req, res, next) => {
    ...
    if (err.statcode === 400 || err.code === "23502" || err.code === "23503") {
        res.status(400).send({ status: 400, error: "Error 400! BAD REQUEST (Code 1)", home: homeurl  });
    } else if (err.statcode === 404) {
        res.status(404).send({ status: 404, error: "Error 404! File Not Found", home: homeurl});
    } else if (err.code.slice(0, 4) === "2350") {
      // 404 - Integrity Constraint Violation
      res.status(400).send({ status: 400, error: "Error 400! BAD REQUEST  (Code 2)", home: homeurl  });
      ...
      res.status(400).send({ status: 400, error: "Error 400! BAD REQUEST  (Code 3)", details: err, home: homeurl  });
    } else if (ec === "53" || ec === "54" || ec === "57") {
      // Handle Internal Server Errors
      res.status(500).send({ status: 500, error: "Error 500 - Internal Server Error", home: homeurl });
    } else {
      next(err);
    }
};
...
exports.testNext404 = (err, req, res, next) => {
    res.status(404).send({status: 404, error: "T / Error 404! File Not Found", home: homeurl});
};
